...
const ProjectCardChild = forwardRef<HTMLDivElement, ProjectCardProps>(
...
ProjectCardChild.displayName = "ProjectCardChild";
...
export const AnimatedFolder: React.FC<AnimatedFolderProps> = ({ title, projects, className, gradient }) => {
    ...
    const backBg = gradient || "linear-gradient(135deg, #2a2a2a 0%, #1a1a1a 100%)";
    const tabBg = gradient || "#1a1a1a";
    const frontBg = gradient || "linear-gradient(135deg, #3a3a3a 0%, #2a2a2a 100%)";
                ...
                <div className="relative flex items-center justify-center mb-4" style={{ height: "160px", width: "200px" }}>
                    ...
                    <div className="absolute" style={{ top: "50%", left: "50%", transform: "translate(-50%, -50%)", zIndex: 20 }}>
                        {previewProjects.map((project, index) => (
                            <ProjectCardChild key={project.id} ref={(el) => { cardRefs.current[index] = el; }} image={project.image} title={project.title} delay={index * 50} isVisible={isHovered} index={index} totalCount={previewProjects.length} onClick={() => handleProjectClick(project, index)} isSelected={hiddenCardId === project.id} />
                        ))}
                    </div>
